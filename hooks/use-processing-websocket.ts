...
function transformPurchaseToTransaction(rawPurchase: any): Transaction {
  const userData =
    rawPurchase.user && typeof rawPurchase.user === "object"
      ? rawPurchase.user
      : null;
  const nestedData =
    rawPurchase.data && typeof rawPurchase.data === "object"
      ? rawPurchase.data
      : null;

  const paymentDetails =
    rawPurchase.payment_details &&
    typeof rawPurchase.payment_details === "object"
      ? rawPurchase.payment_details
      : nestedData?.payment_details &&
          typeof nestedData.payment_details === "object"
        ? nestedData.payment_details
        : null;

  const { previous_balance, new_balance } =
    resolveCreditLedgerBalancesFromWsPayload(rawPurchase, nestedData);

  return {
    id:
      rawPurchase.id ||
      rawPurchase.transaction_id ||
      nestedData?.id ||
      nestedData?.transaction_id ||
      "",
    user_username:
      rawPurchase.user_username ||
      userData?.username ||
      userData?.user_username ||
      nestedData?.user_username ||
      rawPurchase.username ||
      nestedData?.username ||
      "",
    user_email:
      rawPurchase.user_email ||
      userData?.email ||
      userData?.user_email ||
    ...
    bonus_amount:
      rawPurchase.bonus_amount || nestedData?.bonus_amount
        ? String(rawPurchase.bonus_amount || nestedData?.bonus_amount)
        : "",
    status: rawPurchase.status || nestedData?.status || "pending",
    type: "purchase",
    operator: rawPurchase.operator || nestedData?.operator || "",
    payment_method:
      rawPurchase.payment_method ||
      nestedData?.payment_method ||
      rawPurchase.operator ||
      nestedData?.operator ||
      "",
    ...
    previous_winning_balance: String(
      rawPurchase.previous_winning_balance ||
        nestedData?.previous_winning_balance ||
        0,
    ),
    new_winning_balance: String(
      rawPurchase.new_winning_balance || nestedData?.new_winning_balance || 0,
    ),
    unique_id:
      rawPurchase.unique_id ||
      nestedData?.unique_id ||
      rawPurchase.id ||
      nestedData?.id ||
      "",
    ...
    created:
      rawPurchase.created ||
      rawPurchase.created_at ||
      nestedData?.created ||
      nestedData?.created_at ||
      new Date().toISOString(),
    updated:
      rawPurchase.updated ||
      rawPurchase.updated_at ||
      nestedData?.updated ||
...
function transformCashoutToTransaction(rawCashout: any): Transaction {
  const userData =
    rawCashout.user && typeof rawCashout.user === "object"
      ? rawCashout.user
      : null;
  const nestedData =
    rawCashout.data && typeof rawCashout.data === "object"
      ? rawCashout.data
      : null;
  ...
  const paymentDetails =
    rawCashout.payment_details && typeof rawCashout.payment_details === "object"
  ...
  const { previous_balance, new_balance } =
    resolveCreditLedgerBalancesFromWsPayload(rawCashout, nestedData);

  return {
    id:
      rawCashout.id ||
      rawCashout.transaction_id ||
      nestedData?.id ||
      nestedData?.transaction_id ||
      "",
    user_id:
    ...
    bonus_amount:
      rawCashout.bonus_amount || nestedData?.bonus_amount
        ? String(rawCashout.bonus_amount || nestedData?.bonus_amount)
        : "",
    status: rawCashout.status || nestedData?.status || "pending",
    type: "cashout",
    operator: rawCashout.operator || nestedData?.operator || "",
    payment_method:
      rawCashout.payment_method ||
      nestedData?.payment_method ||
      rawCashout.operator ||
      nestedData?.operator ||
      "",
    ...
    new_balance,
    previous_winning_balance: String(
      rawCashout.previous_winning_balance ||
        nestedData?.previous_winning_balance ||
        0,
    ),
    new_winning_balance: String(
      rawCashout.new_winning_balance || nestedData?.new_winning_balance || 0,
    ),
    unique_id:
      rawCashout.unique_id ||
      nestedData?.unique_id ||
      rawCashout.id ||
      nestedData?.id ||
      "",
    ...
    created:
      rawCashout.created ||
      rawCashout.created_at ||
      nestedData?.created ||
      nestedData?.created_at ||
      new Date().toISOString(),
    updated:
      rawCashout.updated ||
      rawCashout.updated_at ||
      nestedData?.updated ||
      nestedData?.updated_at ||
      new Date().toISOString(),
    created_at:
...
function transformActivityToQueue(rawActivity: any): TransactionQueue {
  ...
  const nestedData =
    rawActivity.data && typeof rawActivity.data === "object"
      ? rawActivity.data
      : null;
  ...
  return {
    ...
    game_username:
      rawActivity.get_usergame_username ||
      rawActivity.game_username ||
      rawActivity.data?.username ||
      "",
    ...
    updated_at:
      rawActivity.updated_at || rawActivity.updated || new Date().toISOString(),
  };
}
...
export function useProcessingWebSocket({
  ...
  const connect = useCallback(() => {
    if (!effectiveEnabled || !isAuthenticated) {
      console.log(
        "⚠️ WebSocket connection not enabled, user not authenticated, or user is an agent",
      );
      return;
    }
      ...
      const listeners: WebSocketListeners = {
        ...
        onMessage: (data) => {
          try {
            ...
            // --- Protocol-level messages (heartbeat) ---
            if (message.type === "pong") {
              // Heartbeat response received — connection is alive
              return;
            }
            if (message.type === "ping") {
              // Respond to server-initiated pings with a pong
              if (wsUrlRef.current) {
                websocketManager.send(wsUrlRef.current, {
                  type: "pong",
                  timestamp: Date.now(),
                });
              }
            ...
            console.log(
              "📨 Processing WebSocket message received:",
              message.type,
            );
            ...
            if (message.type === "all_activities") {
              const counts: ProcessingCounts = message.counts || {
                ...
                game_activities_count:
                  message.game_activities_data?.length || 0,
              };
              ...
              if (
                ...
                Array.isArray(message.purchase_data)
              ) {
                message.purchase_data.forEach((rawPurchase) => {
                  const transaction =
                    transformPurchaseToTransaction(rawPurchase);
              ...
              if (
                message.game_activities_data &&
                Array.isArray(message.game_activities_data)
              ) {
              ...
              if (message.counts) {
                console.log(
                  "📊 Counts update received in notification:",
                  message.counts,
                );
              ...
              if (
                message.purchase_data &&
                typeof message.purchase_data === "object" &&
                !Array.isArray(message.purchase_data)
              ) {
                const transaction = transformPurchaseToTransaction(
                  message.purchase_data,
                );
              ...
              if (
                message.cashout_data &&
                typeof message.cashout_data === "object" &&
                !Array.isArray(message.cashout_data)
              ) {
                ...
                if (
                  (message as any).payment_details &&
                  !cashoutData.payment_details
                ) {
                  cashoutData.payment_details = (
                    message as any
                  ).payment_details;
              ...
              const activityType = String(message.activity_type || "")
                .toLowerCase()
                .trim();
              if (
                activityType === "game_activity" ||
                activityType === "gameactivity"
              ) {
                ...
                if (!gameActivityData) {
                  if (
                    ...
                    console.log(
                      "📦 Using message itself as game activity data",
                    );
                  } else {
              ...
              if (message.activity_type === "purchase" && message.data) {
                ...
                if (
                  ...
                  data.game_code ||
                  data.operation_type ||
                  data.type === "recharge_game" ||
                  data.type === "redeem_game" ||
                  data.type === "add_user_game" ||
                  ...
                  if (data.type === "purchase") {
                    ...
                    const transaction =
                      transformCashoutToTransaction(cashoutData);
                    onTransactionUpdate?.(transaction);
                  }
                } else {
        ...
        onMaxReconnectAttemptsReached: () => {
          console.log(
            "⚠️ Processing WebSocket: Connection failed after max attempts",
          );
  ...
  useEffect(() => {
    ...
    // Simple heartbeat signal - just a ping message, only when connected
    const sendHeartbeat = () => {
      if (wsUrlRef.current && websocketManager.isConnected(wsUrlRef.current)) {
