...
const mapHistoryMessage = (msg: RawChatMessage): ChatMessage => {
  ...
  return {
    ...
    userBalance:
      msg.user_balance !== undefined
        ? String(msg.user_balance)
        : msg.balance !== undefined
          ? String(msg.balance)
          : undefined,
...
export function useChatWebSocket({
      ...
      try {
        ...
        const payload: HistoryPayload = {
          messages: Array.isArray(data.messages)
            ? normalizeHistoryMessages(data.messages)
            : [],
          page: resolvedPage,
        ...
        !IS_PROD &&
          console.log("👁️ Player last seen at:", data.player_last_seen_at);
  ...
  const handleBalanceUpdate = useCallback(
    (rawData: RawChatMessage) => {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.userId === Number(playerId) || msg.sender === "player"
              ? {
                  ...msg,
                  userBalance: balanceStr,
                  winningBalance: winningBalanceStr,
                }
              : msg,
          ),
        );
      ...
      if (onBalanceUpdatedRef.current) {
        ...
        if (
          cashoutRaw !== undefined &&
          cashoutRaw !== null &&
          String(cashoutRaw).trim() !== ""
        ) {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      !IS_PROD &&
        console.log("🔌 Connecting to chat WebSocket via manager:", wsUrl);
      ...
      const listeners: WebSocketListeners = {
        onOpen: () => {
          ...
          if (messageQueueRef.current.length > 0) {
            !IS_PROD &&
              console.log(
                `📤 [Chat WS] Processing ${messageQueueRef.current.length} queued messages`,
              );
        ...
        onMessage: (data) => {
          ...
          if (isMessageType) {
            ...
            const sender: "player" | "admin" = isPlayerSender
              ? "player"
              : "admin";
            const timestamp =
              rawData.sent_time ||
            ...
            if (hasMessageContent && isMountedRef.current) {
              setMessages((prev) => {
                ...
                if (prev.some((msg) => msg.id === newMessage.id)) {
                  !IS_PROD &&
                    console.log(
                      "⚠️ [Chat WS] Duplicate by ID, skipping:",
                      newMessage.id,
                    );
                  return prev;
                }
                ...
                if (hasRealId) {
                  ...
                  const filtered = prev.filter((msg) => {
                    ...
                    const timeDiff = Math.abs(
                      new Date(msg.timestamp).getTime() - msgTime,
                    );
              ...
              if (onMessageReceivedRef.current) {
                try {
                  ...
                  console.error(
                    "❌ [Chat WS] Error in onMessageReceived callback:",
                    error,
                  );
                }
              }
            ...
            // Admin's own read receipts have is_player_sender=false and sender_id=adminId
            ...
            if (isMountedRef.current) {
              setConnectionError(
                String(rawData.error || rawData.message || "Unknown error"),
              );
            }
          } else {
  ...
  const markAllAsRead = useCallback(
    (lastMessageId?: string) => {
      ...
      const sent = websocketManager.send(wsUrlRef.current, payload);
      if (sent) {
        !IS_PROD &&
          console.log(
            " Sent mark as read to backend",
            lastMessageId ? `(msg: ${lastMessageId})` : "(all)",
          );
      } else {
        !IS_PROD &&
          console.log("⚠️ Cannot mark as read: WebSocket not connected");
      }
    },
    [adminId],
  );
  ...
  const updateMessagesBalance = useCallback(
    (balance: string, winningBalance: string) => {
      if (!userId) return;

      !IS_PROD &&
        console.log("💰 [Chat WS] Updating balance in messages:", {
          userId,
          balance,
          winningBalance,
        });
