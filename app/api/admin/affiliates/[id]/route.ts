...
export async function GET(request: NextRequest, context: RouteContextParams) {
  ...
  if (!id) {
    return NextResponse.json(
      { status: 'error', message: 'Affiliate ID is required' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/affiliates/${id}/`;
    ...
    return NextResponse.json(
      {
        status: 'error',
        message: error instanceof Error ? error.message : 'Failed to fetch affiliate',
      },
      { status: 500 },
    );
  }
...
export async function PATCH(request: NextRequest, context: RouteContextParams) {
  ...
  try {
    ...
    console.error('❌ Affiliates proxy (PATCH) error:', error);
    return NextResponse.json(
      {
        status: 'error',
        message: error instanceof Error ? error.message : 'Failed to update affiliate',
      },
      { status: 500 },
    );
  }
