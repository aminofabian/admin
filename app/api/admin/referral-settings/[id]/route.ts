...
export async function PATCH(request: NextRequest, context: RouteContextParams) {
  ...
  if (!id) {
    return NextResponse.json(
      { status: 'error', message: 'Referral settings ID is required' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/referral-settings/${id}/`;
    ...
    console.error('❌ Referral-settings proxy (PATCH) error:', error);
    return NextResponse.json(
      {
        status: 'error',
        message: error instanceof Error ? error.message : 'Failed to update referral settings',
      },
      { status: 500 },
    );
  }
