...
export async function GET(request: NextRequest, context: RouteContextParams) {
  ...
  if (!id) {
    return NextResponse.json(
      { status: 'error', message: 'Signup bonus ID is required' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/signup-bonuses/${id}/`;
    ...
    return NextResponse.json(
      {
        status: 'error',
        message: error instanceof Error ? error.message : 'Failed to fetch signup bonus',
      },
      { status: 500 },
    );
  }
...
export async function PATCH(request: NextRequest, context: RouteContextParams) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        status: 'error',
        message: error instanceof Error ? error.message : 'Failed to update signup bonus',
      },
      { status: 500 },
    );
  }
