...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/signup-bonuses/${queryString ? `?${queryString}` : ''}`;

    console.log('🔷 Proxying GET signup-bonuses request:', {
      backendUrl,
      hasAuth: !!authHeader,
    ...
    console.log('📥 Signup-bonuses response status:', response.status);

    return NextResponse.json(data, { status: response.status });
  } catch (error) {
    console.error('❌ Signup-bonuses proxy (GET) error:', error);

    return NextResponse.json(
      {
        status: 'error',
        message: error instanceof Error ? error.message : 'Failed to fetch signup bonuses',
      },
      { status: 500 },
    );
  }
