...
export async function PUT(request: NextRequest) {
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/transfer-bonuses/`;
    ...
    console.error('❌ Transfer-bonuses proxy (PUT) error:', error);
    return NextResponse.json(
      {
        status: 'error',
        message: error instanceof Error ? error.message : 'Failed to update transfer bonuses',
      },
      { status: 500 },
    );
  }
