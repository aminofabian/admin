...
export async function GET(request: NextRequest, context: RouteContextParams) {
  ...
  if (!id) {
    return NextResponse.json(
      { status: 'error', message: 'Transfer bonus ID is required' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/transfer-bonuses/${id}/`;
    ...
    return NextResponse.json(
      {
        status: 'error',
        message: error instanceof Error ? error.message : 'Failed to fetch transfer bonus',
      },
      { status: 500 },
    );
  }
...
export async function PATCH(request: NextRequest, context: RouteContextParams) {
  ...
  try {
    ...
    console.error('❌ Transfer-bonuses proxy (PATCH) error:', error);
    return NextResponse.json(
      {
        status: 'error',
        message: error instanceof Error ? error.message : 'Failed to update transfer bonus',
      },
      { status: 500 },
    );
  }
