...
export async function GET(request: NextRequest, context: RouteContextParams) {
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/meta-capi-events/${id}/`;
    ...
    console.error('❌ Meta CAPI event detail proxy (GET) error:', error);
    return NextResponse.json(
      {
        status: 'error',
        message:
          error instanceof Error ? error.message : 'Failed to fetch Meta CAPI event',
      },
      { status: 500 },
    );
  }
