...
export async function PATCH(request: NextRequest, context: RouteContextParams) {
  ...
  if (!id) {
    return NextResponse.json(
      { status: 'error', message: 'Referral player override ID is required' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/referral-player-overrides/${id}/`;
    ...
    console.error('❌ Referral-player-overrides proxy (PATCH) error:', error);
    return NextResponse.json(
      {
        status: 'error',
        message:
          error instanceof Error ? error.message : 'Failed to update referral player override',
      },
      { status: 500 },
    );
  }
