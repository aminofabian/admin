...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/payment-methods/${queryString ? `?${queryString}` : ''}`;

    console.log('🔷 Proxying GET payment-methods request:', {
      backendUrl,
      hasAuth: !!authHeader,
    ...
    console.error('❌ Payment-methods proxy (GET) error:', error);
