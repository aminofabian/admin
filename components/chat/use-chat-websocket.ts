 ...
 */
const createMapHistoryMessage =
  (playerId: number | null) =>
  (msg: RawHistoryMessage): ChatMessage => {
    ...
    console.log(
      `📩 MSG ${msg.id}: sender_id=${msg.sender_id}, playerId=${playerId}, is_player_sender=${msg.is_player_sender}, sender="${msg.sender}" → ${sender}`,
    );

    const timestamp =
      msg.sent_time ?? msg.timestamp ?? new Date().toISOString();
    ...
    return {
      ...
      userBalance:
        creditBalance !== undefined && creditBalance !== null
          ...
          : undefined,
      winningBalance:
        winningBalance !== undefined && winningBalance !== null
...
export function useChatReset({
  ...
  const requestHistory = useCallback(
    async (page: number): Promise<HistoryPayload | null> => {
      if (!chatId && !userId) {
        return null;
      }
      ...
      if (cached) {
        if (!IS_PROD) {
          console.log(
            ` Cache HIT for page ${page} - returning ${cached.messages.length} messages`,
          );
        }
        return {
      ...
      try {
            ...
            lastMsg:
              data.messages.length > 1
                ? {
                    ...
                    is_player_sender:
                      data.messages[data.messages.length - 1].is_player_sender,
                    sender: data.messages[data.messages.length - 1].sender,
                    sender_id:
                      data.messages[data.messages.length - 1].sender_id,
        ...
        const payload: HistoryPayload = {
          messages: Array.isArray(data.messages)
            ? normalizeHistoryMessages(data.messages, userId)
            : [],
          page: resolvedPage,
    ...
    async (page: number, mode: HistoryMergeMode): Promise<number> => {
      ...
      if (cached) {
        ...
        if (cached.page < cached.totalPages) {
          void messageCache.prefetchPage(
            chatId,
            userId,
            cached.page + 1,
            requestHistory,
      ...
      try {
        ...
        if (historyRequestRef.current !== requestId) {
          if (!IS_PROD)
            console.log("⚠️ Ignoring stale message history response");
          return 0;
        }
        ...
        if (payload.page < payload.totalPages) {
          ...
          void messageCache.prefetchPages(
            chatId,
            userId,
            nextPages,
            requestHistory,
  ...
  const fetchPurchaseHistory = useCallback(async () => {
    ...
    try {
      ...
      if (data.messages && Array.isArray(data.messages)) {
        ...
        const purchases: ChatMessage[] = data.messages.map(
          (msg: RawHistoryMessage) => {
        ...
        if (!IS_PROD)
          console.log(
            ` Loaded ${purchases.length} purchase records (total: ${data.total_count || purchases.length})`,
          );
        if (purchaseRequestRef.current !== requestId) {
          if (!IS_PROD)
            console.log("⚠️ Ignoring stale purchase history response");
          return;
        }
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onmessage = (event) => {
        if (activeConnectionKeyRef.current !== connectionKey) {
          if (!IS_PROD)
            console.log("⚠️ Ignoring message from stale WebSocket connection");
          return;
        }
        ...
        try {
          ...
          if (messageType === "message") {
            ...
            // Extract credit balance (try multiple field names)
            const creditBalance =
              rawData.user_balance ?? rawData.balance ?? rawData.player_bal;
            // Extract winning balance (try multiple field names)
            const winningBalance =
              rawData.winning_balance ?? rawData.player_winning_bal;
            ...
            if (hasMessageContent) {
              ...
              setMessages((prev) => {
                ...
                if (!IS_PROD)
                  console.log(
                    `📝 Messages state updated: ${prev.length} -> ${updated.length} messages`,
                  );
                return updated;
              });
              ...
              if (onMessageReceivedRef.current) {
                ...
                if (!IS_PROD)
                  console.warn("⚠️ onMessageReceived callback is not defined");
              }
            } else {
              if (!IS_PROD)
                console.log(
                  "ℹ️ Message has no content, skipping UI update but checking for balance info",
                );
            }
            ...
            if (rawData.user_balance !== undefined) {
              if (!IS_PROD)
                console.log("💰 User balance update:", rawData.user_balance);
            ...
            messageType === "mark_message_as_read" ||
            messageType === "read"
          ) {
            ...
            // Guard: ignore read receipts from ourselves (server echo)
            if (!isPlayerSender && String(senderId) === String(adminId)) {
              return;
            }

            if (messageId) {
              // Mark specific message and all earlier ones from the target sender as read
              if (!IS_PROD) console.log(" Message marked as read:", messageId);
              const targetSender: "player" | "admin" = isPlayerSender
                ? "admin"
                : "player";
              setMessages((prev) => {
                ...
                return prev.map((msg, idx) => {
                  if (idx <= targetIndex && msg.sender === targetSender) {
              ...
              if (!IS_PROD)
                console.log(
                  ` Marking all ${targetSender} messages as read (read by ${isPlayerSender ? "player" : "admin"} with sender_id: ${senderId})`,
                );
              setMessages((prev) =>
                prev.map((msg) => {
            ...
            if (!IS_PROD)
              console.log(
                `🟢 Live status: ${rawData.username || `Player ${playerId}`} is ${isActive ? "ONLINE" : "OFFLINE"}`,
              );
            ...
            setConnectionError(
              rawData.error || rawData.message || "Unknown error",
            );
          } else {
            if (!IS_PROD)
      ...
      ws.onclose = (event) => {
        ...
        if (event.code === 1006 || event.code === 1001) {
          if (!IS_PROD)
            console.log(
              "⚠️ Backend WebSocket not available, using REST API fallback",
            );
          return;
        }
        ...
        if (enabled && reconnectAttemptsRef.current < maxReconnectAttempts) {
          ...
          if (!IS_PROD)
            console.log(
              "⚠️ Max reconnection attempts reached, using REST API fallback",
            );
        }
      };
    } catch (error) {
    ...
    (text: string) => {
      ...
      if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
        try {
          ...
          if (!IS_PROD)
            console.log(
              "📤 Sent message via WebSocket - waiting for confirmation",
            );
          return;
        } catch (error) {
          console.error(
            "❌ Failed to send via WebSocket, trying REST API:",
            error,
          );
        }
      }
  ...
  const markAllAsRead = useCallback(
    (lastMessageId?: string) => {
      if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
        if (!IS_PROD)
          console.log("⚠️ Cannot mark all as read: WebSocket not connected");
        return;
      }
  ...
  const updateMessagePinnedState = useCallback(
