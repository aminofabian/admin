...
interface AddGameDrawerProps {
  ...
  onSubmit: (data: { game_id: number }) => Promise<void>;
...
export function AddGameDrawer({
  ...
  return (
    <div className="fixed inset-0 z-[110] overflow-hidden">
      ...
      <div
        className={`fixed inset-y-0 right-0 z-[110] w-full sm:max-w-lg bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 shadow-2xl transition-transform duration-300 ease-in-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex h-full flex-col">
          <div className="sticky top-0 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-950/20 dark:to-indigo-950/20 border-b border-gray-200 dark:border-gray-800 px-6 py-5 flex items-center justify-between z-10 backdrop-blur-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">Add game</h2>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-0.5">Enqueue game for this player (admin)</p>
              </div>
            </div>
            <button
              onClick={onClose}
            ...
            <form id="add-game-form" onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
                ...
                {!isLoadingGames && (
                  <p className="mt-1 text-xs text-muted-foreground">
                    {availableGames.length === 0 ? (
                      <span className="text-amber-600 dark:text-amber-400">Player already has all listed games</span>
                    ) : (
                      <>
                        {availableGames.length} available
                        {playerGames.length > 0 ? ` · ${playerGames.length} on player` : ''}
                      </>
                    )}
                  </p>
                )}
              ...
              <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground mb-1">Player</p>
                    <p className="text-xs text-muted-foreground">
                      <span className="font-medium text-foreground">{playerUsername}</span>
                    </p>
                    <p className="text-xs text-muted-foreground mt-2">
                      Creation runs in the background. Check game activity queues for status.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="sticky bottom-0 z-10 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 px-6 py-5 flex items-center justify-end gap-3 shadow-lg">
            <Button variant="ghost" onClick={onClose} disabled={isSubmitting} className="px-6 py-2.5 font-medium transition-all hover:bg-gray-100 dark:hover:bg-gray-800">
              Cancel
            </Button>
            <Button
              type="submit"
