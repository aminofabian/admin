export { ManagerDashboard } from './manager-dashboard';
export { ManagerPlayersSection } from './manager-players-section';
export { ManagerGamesSection } from './manager-games-section';
export { ManagerBonusesSection } from './manager-bonuses-section';
export { default as ManagerSettingsSection } from './manager-settings-section';
export { ManagerPlayerDetail } from './manager-player-detail';
export { ManagerBannersSection } from './manager-banners-section';

