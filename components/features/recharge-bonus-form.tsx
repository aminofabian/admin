...
import type { UpdateBonusRequest, RechargeBonus } from '@/types';

interface RechargeBonusFormProps {
  onSubmit: (data: UpdateBonusRequest) => Promise<void>;
  onCancel: () => void;
  isLoading?: boolean;
  initialData?: RechargeBonus;
}

export function RechargeBonusForm({ 
  onSubmit, 
  onCancel, 
  isLoading = false,
  initialData,
}: RechargeBonusFormProps) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="w-full px-3 py-2 border rounded-md bg-gray-50 dark:bg-gray-900 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100">
            {initialData?.bonus_type === 'percentage' ? 'Percentage' : 'Fixed Amount'}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder={initialData?.bonus_type === 'percentage' ? 'Enter percentage (e.g., 10)' : 'Enter amount (e.g., 50)'}
              min="0"
              max={initialData?.bonus_type === 'percentage' ? '100' : undefined}
              step={initialData?.bonus_type === 'percentage' ? '0.1' : '0.01'}
            />
            {initialData?.bonus_type === 'percentage' && (
