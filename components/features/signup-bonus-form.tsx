...
import type { UpdateBonusRequest, SignupBonus } from '@/types';

interface SignupBonusFormProps {
  onSubmit: (data: UpdateBonusRequest) => Promise<void>;
  onCancel: () => void;
  isLoading?: boolean;
  initialData?: SignupBonus;
}

export function SignupBonusForm({ 
  onSubmit, 
  onCancel, 
  isLoading = false,
  initialData,
}: SignupBonusFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.bonus || formData.bonus <= 0) {
      newErrors.bonus = 'Bonus amount is required and must be greater than 0';
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-blue-700 dark:text-blue-400">
              You can only update the fixed bonus amount and active status. Bonus label cannot be changed.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Bonus label cannot be changed</p>
        </div>

        {/* Bonus Type Display - Always Fixed */}
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            Bonus Type
          </label>
