...
import { Table, TableHeader, TableHead, TableBody, TableRow, TableCell, Pagination, SearchInput, Badge, Button, Drawer, Modal, useToast } from '@/components/ui';
...
export function AgentsSection() {
  ...
  const handleUpdateAgent = async (id: number, data: UpdateUserRequest) => {
    ...
    try {
      const agent = agents?.results?.find(a => a.id === id);
      if (!agent) {
        throw new Error('Agent not found');
      }
      
      const newStatus = data.is_active !== undefined ? data.is_active : agent.is_active;
      const isDeactivating = agent.is_active && !newStatus;
      ...
      const statusVerb = newStatus ? 'activated' : 'deactivated';
      addToast({
        type: 'success',
        title: isDeactivating ? 'Agent deactivated' : 'Agent activated',
        description: `"${agent.username}" has been ${statusVerb} successfully!`,
      });
    } catch (error) {
