...
export function PlayerViewModal({ isOpen, onClose, player }: PlayerViewModalProps) {
  ...
  const creditBalance = useMemo(() => formatCurrency(selectedPlayer?.balance ?? 0), [selectedPlayer?.balance]);
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Player Details" size="md">
      <div className="space-y-6">
        <section className="flex items-center gap-4 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-950/30 dark:to-indigo-950/30 rounded-xl p-4 border border-purple-200 dark:border-purple-800/50">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{selectedPlayer.username}</h3>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-gray-200 dark:border-white/10">
            ...
            <p className="text-gray-900 dark:text-gray-100 break-all">{selectedPlayer.email}</p>
          </div>
          <div className="bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-gray-200 dark:border-white/10">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/40 dark:to-purple-950/40 rounded-lg p-4 border border-indigo-200 dark:border-indigo-800/50 shadow-sm">
            ...
            <p className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{creditBalance}</p>
          </div>
          <div className="bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-950/40 dark:to-green-950/40 rounded-lg p-4 border border-emerald-200 dark:border-emerald-800/50 shadow-sm">
            ...
            <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{winningBalance}</p>
          </div>
        </section>

        <section className="bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-950/20 dark:to-indigo-950/20 rounded-xl p-5 border border-purple-200 dark:border-purple-800/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white/60 dark:bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-purple-200/50 dark:border-purple-700/50 hover:shadow-md transition-all duration-300">
              ...
              {isLoadingDetails ? (
                <div className="h-7 bg-purple-300/30 dark:bg-purple-700/30 rounded w-24 animate-pulse" />
              ) : (
                <p className="text-xl font-bold text-purple-600 dark:text-purple-400 transition-all duration-300">{purchasesTotal}</p>
              )}
            </div>
            <div className="bg-white/60 dark:bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-indigo-200/50 dark:border-indigo-700/50 hover:shadow-md transition-all duration-300">
              ...
              {isLoadingDetails ? (
                <div className="h-7 bg-indigo-300/30 dark:bg-indigo-700/30 rounded w-24 animate-pulse" />
              ) : (
                <p className="text-xl font-bold text-indigo-600 dark:text-indigo-400 transition-all duration-300">{cashoutsTotal}</p>
              )}
            </div>
            <div className="bg-white/60 dark:bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-violet-200/50 dark:border-violet-700/50 hover:shadow-md transition-all duration-300">
              ...
              {isLoadingDetails ? (
                <div className="h-7 bg-violet-300/30 dark:bg-violet-700/30 rounded w-24 animate-pulse" />
              ) : (
                <p className="text-xl font-bold text-violet-600 dark:text-violet-400 transition-all duration-300">{transfersTotal}</p>
              )}
            </div>
          </div>
        </section>

        <section className="bg-white/50 dark:bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-gray-200 dark:border-white/10">
          <div className="flex items-center gap-2 mb-2">
