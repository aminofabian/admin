...
export function DetailsAmountBox({ amount, bonus, variant = 'positive' }: DetailsAmountBoxProps) {
  ...
  const dividerColor = isNegative
    ? 'bg-red-300 dark:bg-red-700'
    : 'bg-green-300 dark:bg-green-700';
  const bonusColor = isNegative
    ? 'text-red-500 dark:text-red-300'
    : 'text-emerald-600 dark:text-emerald-400';

  return (
    <div className={`p-2 ${containerClass} border rounded`}>
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <div className="text-[10px] text-muted-foreground mb-0.5">Amount</div>
          <div className={`text-base font-bold ${amountColor}`}>{amount}</div>
        </div>
        {bonus && (
          <>
            <div className={`h-6 w-px ${dividerColor} mx-2`} />
            <div className="flex-1 text-right">
              <div className="text-[10px] text-muted-foreground mb-0.5">Bonus</div>
              <div className={`text-base font-bold ${bonusColor}`}>{bonus}</div>
            </div>
          </>
        )}
      </div>
