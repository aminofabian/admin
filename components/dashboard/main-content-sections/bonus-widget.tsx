...
export function BonusWidget() {
  ...
  const groupedPurchaseBonuses = useMemo(
    () => groupPurchaseBonusesForDisplay(purchaseBonusesList),
    [purchaseBonusesList],
  );
                  ...
                  {category.id === 'topup' && (
                    <div className="space-y-2.5">
                      ...
                      {purchaseBonusesList.length === 0 ? (
                        ...
                        groupedPurchaseBonuses.map((group) => (
                          <div key={group.category} className="space-y-1.5 mb-3 last:mb-0">
                            <p className="text-xs font-medium text-muted-foreground px-0.5">{group.category}</p>
                            {group.items.map((bonus) => (
                              <div
                                key={bonus.id}
                                className="flex justify-between items-center p-2 bg-secondary/50"
                              >
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-sm text-foreground font-medium">
                                    {formatPaymentMethod(bonus.topup_method)}
                                  </span>
                                </div>
