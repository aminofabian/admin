...
export function LivePlayersWidget({ stats, statsLoading }: LivePlayersWidgetProps) {
  ...
  const isLoading = statsLoading || (canViewPlayers && playersLoading);

  // Show placeholder if no permission
  if (!canViewPlayers) {
    return (
      <div className="bg-card rounded-xl p-4 xl:p-5 border border-border shadow-sm">
  ...
  if (playersError) {
    return (
      <div className="bg-card rounded-xl p-4 xl:p-5 border border-border shadow-sm">
        ...
        <ErrorState message={playersError} />
      </div>
    );
  }
    ...
    <div className="bg-card rounded-xl p-4 xl:p-5 border border-border shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="text-center">
        ...
        {playerData.length > 0 && (
          <div className="grid grid-cols-2 gap-2 text-center pt-3 border-t border-border">
            <div className="bg-muted/30 rounded-lg p-2">
              <div className="text-sm font-semibold text-foreground">{playersWithBalance}</div>
              <div className="text-xs text-muted-foreground">with balance</div>
            </div>
            <div className="bg-muted/30 rounded-lg p-2">
              <div className="text-sm font-semibold text-foreground">{playersWithWinnings}</div>
              <div className="text-xs text-muted-foreground">with winnings</div>
            </div>
          </div>
        )}
        ...
        <div className="text-xs text-muted-foreground/80 mt-3 pt-3 border-t border-border flex items-center justify-center gap-2">
          ...
          {players?.results ? 'Live Player Data' : 'Dashboard Stats'}
        </div>
      </div>
    </div>
  );
